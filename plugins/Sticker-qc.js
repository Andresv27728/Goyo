...
text = m.quoted.text;
} else return conn.reply(m.chat, '🚩 Ingresa el texto para el sticker.', m, rcanal);
if (!text) return conn.reply(m.chat, '🚩 Ingresa el texto para el sticker.', m, rcanal); // Should be caught by the one above, but kept for safety.
const who = m.mentionedJid && m.mentionedJid[0] ? m.mentionedJid[0] : m.fromMe ? conn.user.jid : m.sender;
const mentionRegex = new RegExp(`@${who.split('@')[0].replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*`, 'g');
const mishi = text.replace(mentionRegex, '');
if (mishi.length > 40) return conn.reply(m.chat, '🚩 El texto no puede exceder los 40 caracteres.', m, rcanal);
