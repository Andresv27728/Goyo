...
let handler = async (m, { conn, usedPrefix }) => {
    ...
    let str;
    if (m.mentionedJid.length > 0) {
        str = `\`${name2}\` está seduciendo a \`${name || who}\`.`; // Usamos nombre agendado o número si no está agendado
    } else if (m.quoted) {
        str = `\`${name2}\` está seduciendo a  \`${name || who}\` ( ͡° ͜ʖ ͡°).`; // Mensaje cuando se cita a otro usuario
    } else {
...
handler.help = ['seduce @tag', 'seducir @tag'];
handler.tags = ['emox'];
